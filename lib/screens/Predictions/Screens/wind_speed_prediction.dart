...
class _WindSpeedPredictionState extends State<WindSpeedPrediction> {
  ...
  void getService() async {
    ...
    try {
      Map<String, dynamic> response = await apiService.postData('windSpeed', requestData);

      Map<String, double> fetchedWindSpeedData = Map<String, double>.from(response['data']);
                      ...
                      sideTitles: SideTitles(
                        ...
                        getTitlesWidget: (value, meta) {
                          ...
                          return SideTitleWidget(
                            ...
                            child: Text(
                              windSpeedData.keys.toList()[value.toInt()],
                              style: style,
                            ),
                          );
                        },
                  ...
                  lineBarsData: [
                    LineChartBarData(
                      spots: windSpeedData.entries
                          .map((entry) => FlSpot(
                        windSpeedData.keys.toList().indexOf(entry.key).toDouble(),
                        entry.value,
                      ))
                          .toList(),
                      isCurved: true,
                      color: Colors.blue, // Cambia el color según lo que necesites
                      barWidth: 4,
